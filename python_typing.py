...
# strings could generate problems.
# For example, the two cases above that generate type checker errors could be
...
# should use `Self` as the return annotation.
# If `Foo.return_self` was annotated as returning `"Foo"`, then the type
...
# this class with one
# or more type variables. For example, a generic mapping type might be defined
# as:
KT = TypeVar("KT")
VT = TypeVar("VT")
...
# variable tuple, in contrast, allows parameterization with an _arbitrary_
# number of types by acting like an _arbitrary_ number of type variables
...
# Note the use of the unpacking operator `*` in `tuple[T, *Ts]`. Conceptually,
# you can think of `Ts` as a tuple of type variables `(T1, T2, ...)`.
# `tuple[T, *Ts]` would then become `tuple[T, *(T1, T2, ...)]`, which is
# equivalent to `tuple[T, T1, T2, ...]`. (Note that in older versions of
# Python, you might see this written using [Unpack](#typing.Unpack) instead,
# as `Unpack[Ts]`.) Type variable tuples must _always_ be unpacked. This helps
